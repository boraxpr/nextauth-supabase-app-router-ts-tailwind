      ...
      <div className="shadow-lg ">
        <div className="flex justify-end my-5">
          <a href="/quotations/create">
            <Button
              variant="default"
              className="py-2 px-24 rounded-md no-underline border bg-primary text-primary-foreground"
            >
              Create Quotation
            </Button>
          </a>
                ...
                quotations.map((quotation) => (
                  <tr key={quotation.doc_num}>
                    ...
                    <td className="p-2 text-center">
                      <div>
                        <Link
                          href={`/quotations/details/${quotation.doc_num}`}
                          className="inline-flex justify-center items-center text-sm border w-8 h-8 shadow-lg rounded-lg bg-btn-background hover:bg-btn-background-hover"
                        >
                          <span>•••</span>
                        </Link>
                      </div>
      ...
      <div className="flex flex-row justify-center my-1">
        <div className="space-x-5">
          {(page != 1 || page <= totalPages) && (
            <Link href={`/quotations?page=${Number(page) - 1}`}>Prev</Link>
          )}
          {(page < totalPages || page == 1) && (
            <Link href={`/quotations?page=${(Number(page) + 1).toString()}`}>
              Next
            </Link>
          )}
        </div>
